...
class CasinoStartMenu:
    ...
    def draw(self):
        ...
        for button in self.buttons:
            color = BUTTON_HOVER if button["rect"].collidepoint(
                mouse_pos) else BUTTON_COLOR

            pygame.draw.rect(self.screen, color,
                             button["rect"], border_radius=12)
            pygame.draw.rect(self.screen, BORDER_COLOR,
                             button["rect"], width=2, border_radius=12)
        ...
        title_rect = title.get_rect(
            center=(self.panel_rect.centerx, self.panel_rect.y + 70))
